...
async function loadData(){
  ...
  renderCategories();
  showSection('hero'); // start at hero
}
...
function renderMore(){
  ...
  grid.insertAdjacentHTML('beforeend', slice.map(p => `
    <article class="card">
      <img src="${p.image}" alt="${p.title}" loading="lazy" onerror="this.src='img/products/p001.webp'">
      <h3>${p.title}</h3>
  ...
  if(moreBtn){
    moreBtn.style.display = (start + PAGE_SIZE < list.length) ? '' : 'none';
  }
}
...
function activateProductsView(){ state.q=''; resetAndRenderProducts(); }
